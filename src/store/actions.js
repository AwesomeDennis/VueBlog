...
export default{
    login({commit}, payload){
        console.log(payload)
        return Vue.http.post('/api/login', payload).then(response => {
        ...
        // Vue.http.post("./src/store/a.json",payload).then((response)=>{console.log(response.data)})
    },
    getArticles: ({commit}) => {
    ...
    getLinks: ({commit}) => {
        return Vue.http.get('/api/getLinks')
            .then(response => {
